...
contract Tracking {
    ...
    /*
    function getTransactions() public view returns (Shipment[] memory) {
         uint256 len = tyepShipments.length;
    Shipment[] memory result = new Shipment[](len);
    for (uint i = 0; i < len; i++){
       // result[i] = tyepShipments[i];
    }
    return result;
    }
}  */
